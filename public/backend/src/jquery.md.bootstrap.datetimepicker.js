...
<td class="border-0" style="{{monthTdStyle}}" {{monthTdAttribute}} data-td-month>
    ...
    function isWithinMdModal($element) {
        return (
            $element.parents(".modal" + mdDatePickerElementSelector + ":first")
                .length > 0
        );
    }
    ...
    function getPopoverDescriber($element) {
        ...
        var $popoverDescriber = $element.parents(
            mdDatePickerFlagSelector + ":first"
        ); // inline
        // not inline
        if ($popoverDescriber.length <= 0) {
            $popoverDescriber = $element.parents(
                mdDatePickerElementSelector + ":first"
            );
            $popoverDescriber = $(
                '[aria-describedby="' + $popoverDescriber.attr("id") + '"]'
            );
        }
    ...
    function getSetting1($element) {
        ...
        if (isWithinMdModal($element)) {
            var buttonSelector = $element
                .parents("[data-buttonselector]:first")
                .attr("data-buttonselector");
            return $('[data-uniqueid="' + buttonSelector + '"]').data(
                mdPluginName
            );
        } else {
    ...
    function setPopoverHeaderHtml($element, isInLine, htmlString) {
        ...
        if (!isInLine) {
            if (isPopoverDescriber($element)) {
                getPopover($element)
                    .find('[data-name="mds-datetimepicker-title"]')
                    .html(htmlString);
            } else {
                $element
                    .parents(mdDatePickerElementSelector + ":first")
                    .find('[data-name="mds-datetimepicker-title"]')
                    .html(htmlString);
            }
        } else {
            var $inlineTitleBox = $element
                .parents(mdDatePickerFlagSelector + ":first")
                .find('[data-name="dateTimePickerYearsButtonsContainer"]');
    ...
    function updateCalendarHtml1($element, setting) {
        var calendarHtml = getDateTimePickerHtml(setting),
            $container = setting.inLine
                ...
                : $element.parents(
                      '[data-name="mds-datetimepicker-body"]:first'
                  );
        setPopoverHeaderHtml(
            $element,
            setting.inLine,
            $(calendarHtml).find("[data-selecteddatestring]").text().trim()
        );
    ...
    function getSelectedDateTimeTextFormatted(setting) {
        ...
        if (
            ...
            return (
                getDateTimeString(
                    !setting.isGregorian
                        ? getDateTimeJsonPersian1(
                              setting.rangeSelectorStartDate
                          )
                        : getDateTimeJson1(setting.rangeSelectorStartDate),
                    setting.textFormat,
                    setting.isGregorian,
                    setting.englishNumber
                ) +
                " - " +
                getDateTimeString(
                    !setting.isGregorian
                        ? getDateTimeJsonPersian1(setting.rangeSelectorEndDate)
                        : getDateTimeJson1(setting.rangeSelectorEndDate),
                    setting.textFormat,
                    setting.isGregorian,
                    setting.englishNumber
                )
        ...
        return getDateTimeString(
            !setting.isGregorian
                ...
                : getDateTimeJson1(setting.selectedDate),
            setting.textFormat,
            setting.isGregorian,
            setting.englishNumber
        );
    ...
    function setSelectedData(setting) {
        ...
        if ($targetDate.length > 0) {
            switch ($targetDate[0].tagName.toLowerCase()) {
                case "input":
                    $targetDate.val(
                        toEnglishNumber(getSelectedDateTimeFormatted(setting))
                    );
                    triggerChangeCalling = true;
    ...
    function setSelectedRangeData(setting) {
        ...
        if (!startDateTimeObject)
            throw new Error(
                `Start Date of '${setting.targetTextSelector}' is not valid for range selector`
            );
        if (!endDateTimeObject)
            throw new Error(
                `End Date of '${setting.targetTextSelector}' is not valid for range selector`
            );
    ...
    function toPersianNumber(inputNumber1) {
        ...
        if (!str1) return "";
        str1 = str1.replace(/0/gim, "0");
        str1 = str1.replace(/1/gim, "1");
        str1 = str1.replace(/2/gim, "2");
        str1 = str1.replace(/3/gim, "3");
        str1 = str1.replace(/4/gim, "4");
        str1 = str1.replace(/5/gim, "5");
        str1 = str1.replace(/6/gim, "6");
        str1 = str1.replace(/7/gim, "7");
        str1 = str1.replace(/8/gim, "8");
        str1 = str1.replace(/9/gim, "9");
        return str1;
    }
    ...
    function toEnglishNumber(inputNumber2) {
        ...
        if (!str) return "";
        str = str.replace(/۰/gim, "0");
        str = str.replace(/۱/gim, "1");
        str = str.replace(/۲/gim, "2");
        str = str.replace(/۳/gim, "3");
        str = str.replace(/۴/gim, "4");
        str = str.replace(/۵/gim, "5");
        str = str.replace(/۶/gim, "6");
        str = str.replace(/۷/gim, "7");
        str = str.replace(/۸/gim, "8");
        str = str.replace(/۹/gim, "9");
        return str;
    }
    ...
    function addMonthToDateTime(dateTime, addedMonth, isGregorian) {
        ...
        if (!isGregorian) {
            ...
            dateTimeJson = addMonthToDateTimeJson(
                dateTimeJson,
                addedMonth,
                isGregorian
            );
    ...
    function convertToNumber1(dateTimeJson) {
        return Number(
            zeroPad(dateTimeJson.year) +
                zeroPad(dateTimeJson.month) +
                zeroPad(dateTimeJson.day)
        );
    }
    ...
    function convertToNumber4(dateTime) {
        return Number(
            zeroPad(dateTime.getFullYear()) +
                zeroPad(dateTime.getMonth()) +
    ...
    function getDateTime2(dateTimeJsonPersian) {
        ...
        var gregorian = toGregorian(
            dateTimeJsonPersian.year,
            dateTimeJsonPersian.month,
            dateTimeJsonPersian.day
        );
        return new Date(
            gregorian.gy,
            gregorian.gm - 1,
            gregorian.gd,
            dateTimeJsonPersian.hour,
            dateTimeJsonPersian.minute,
            dateTimeJsonPersian.second
        );
    }
    ...
    function getDateTimeJsonPersian2(
        yearPersian,
        monthPersian,
        dayPersian,
        hour,
        ...
        if (!isNumber(second)) second = 0;
        var gregorian = toGregorian(yearPersian, monthPersian, dayPersian);
        return getDateTimeJsonPersian1(
            new Date(
                gregorian.gy,
                gregorian.gm - 1,
                gregorian.gd,
    ...
    function getDateTimeString(
        ...
        format = format.replace(/yyyy/gm, dateTimeJson.year);
        format = format.replace(/yy/gm, dateTimeJson.year % 100);
        format = format.replace(
            /MMMM/gm,
            getMonthName(dateTimeJson.month - 1, isGregorian)
        );
        format = format.replace(/MM/gm, zeroPad(dateTimeJson.month));
        format = format.replace(/M/gm, dateTimeJson.month);
        format = format.replace(
            /dddd/gm,
            getWeekDayName(dateTimeJson.dayOfWeek, isGregorian)
        );
        format = format.replace(/dd/gm, zeroPad(dateTimeJson.day));
        format = format.replace(/d/gm, dateTimeJson.day);
        format = format.replace(/HH/gm, zeroPad(dateTimeJson.hour));
        format = format.replace(/H/gm, dateTimeJson.hour);
        format = format.replace(
            /hh/gm,
            zeroPad(getShortHour(dateTimeJson.hour))
        );
        format = format.replace(/h/gm, zeroPad(dateTimeJson.hour));
        format = format.replace(/mm/gm, zeroPad(dateTimeJson.minute));
        format = format.replace(/m/gm, dateTimeJson.minute);
        format = format.replace(/ss/gm, zeroPad(dateTimeJson.second));
        format = format.replace(/s/gm, dateTimeJson.second);
        format = format.replace(
            /fff/gm,
            zeroPad(dateTimeJson.millisecond, "000")
        );
        format = format.replace(/ff/gm, zeroPad(dateTimeJson.millisecond / 10));
        format = format.replace(/f/gm, dateTimeJson.millisecond / 100);
        format = format.replace(
            /tt/gm,
            getAmPm(dateTimeJson.hour, isGregorian)
        );
        format = format.replace(
            /t/gm,
            getAmPm(dateTimeJson.hour, isGregorian)[0]
        );
    ...
    function parsePersianDateTime(
        persianDateTimeInString,
        ...
        var month = 0,
            ...
            millisecond = 0,
            amPmEnum = amPm.none,
            containMonthSeparator = dateSeparatorPattern.test(
                persianDateTimeInString
            );

        persianDateTimeInString = persianDateTimeInString.replace(
            /&nbsp;/gim,
            " "
        );
        persianDateTimeInString = persianDateTimeInString.replace(
        ...
        persianDateTimeInString = persianDateTimeInString.replace(/\\/gim, "-");
        persianDateTimeInString = persianDateTimeInString.replace(/ك/gim, "ک");
        persianDateTimeInString = persianDateTimeInString.replace(/ي/gim, "ی");
        persianDateTimeInString = persianDateTimeInString.replace(
            dateSeparatorPattern,
            "-"
        );
        ...
        if (persianDateTimeInString.indexOf("ق.ظ") > -1) amPmEnum = amPm.AM;
        else if (persianDateTimeInString.indexOf("ب.ظ") > -1)
            amPmEnum = amPm.PM;

        if (persianDateTimeInString.indexOf(":") > -1) {
            // رشته ورودی شامل ساعت نیز هست
            persianDateTimeInString = persianDateTimeInString.replace(
                /-*:-*/gim,
                ":"
            );
            hour = persianDateTimeInString
                .match(/-\d{1,2}(?=:)/gim)[0]
                .replace(/\D+/, "");
            var minuteAndSecondAndMillisecondMatch =
                persianDateTimeInString.match(/:\d{1,2}(?=:?)/gim);
            minute = minuteAndSecondAndMillisecondMatch[0].replace(/\D+/, "");
            if (minuteAndSecondAndMillisecondMatch[1] != undefined)
                second = minuteAndSecondAndMillisecondMatch[1].replace(
                    /\D+/,
                    ""
                );
            if (minuteAndSecondAndMillisecondMatch[2] != undefined)
                millisecond = minuteAndSecondAndMillisecondMatch[2].replace(
                    /\D+/,
                    ""
                );
        }
        ...
        if (containMonthSeparator) {
            var monthDayMath = persianDateTimeInString.match(
                /-\d{1,2}(?=-\d{1,2}[^:]|-)/gim
            );
            ...
            // بدست آوردن سال
            year = persianDateTimeInString
                .match(/-\d{2,4}(?=-\d{1,2}[^:])/gim)[0]
                .replace(/\D+/, "");
        } else {
            for (var i = 1; i < 12; i++) {
                var persianMonthName = getMonthName(i - 1, false);
                if (persianDateTimeInString.indexOf(persianMonthName) > -1)
                    continue;
                month = i;
                break;
            }
            ...
            // بدست آوردن روز
            var dayMatch = persianDateTimeInString.match(/-\d{1,2}(?=-)/gim);
            if (dayMatch != null) {
            ...
            // بدست آوردن سال
            var yearMatch = persianDateTimeInString.match(/-\d{4}(?=-)/gim);
            if (yearMatch != null) year = yearMatch[0].replace(/\D+/, "");
            else {
                yearMatch = persianDateTimeInString.match(/-\d{2,4}(?=-)/gim);
                if (yearMatch != null) year = yearMatch[0].replace(/\D+/, "");
            }
        }
    ...
    function getYearsToSelectHtml(setting, yearToStart) {
        ...
        if (setting.isGregorian) {
            ...
            disableBeforeDateTimeJson = !setting.disableBeforeDate
                ? undefined
                : getDateTimeJson1(setting.disableBeforeDate);
            disableAfterDateTimeJson = !setting.disableAfterDate
                ? undefined
                : getDateTimeJson1(setting.disableAfterDate);
        } else {
            selectedDateTimeToShowJson =
                getDateTimeJsonPersian1(selectedDateToShow);
            todayDateTimeJson = getDateTimeJsonPersian1(new Date());
            disableBeforeDateTimeJson = !setting.disableBeforeDate
                ? undefined
                : getDateTimeJsonPersian1(setting.disableBeforeDate);
            disableAfterDateTimeJson = !setting.disableAfterDate
                ? undefined
                : getDateTimeJsonPersian1(setting.disableAfterDate);
        }
        ...
        if ((setting.fromDate || setting.toDate) && setting.groupId) {
            ...
            if (setting.fromDate) {
                ...
                disableAfterDateTimeJson = !toDateSelectedDate
                    ? undefined
                    : setting.isGregorian
                    ? getDateTimeJson1(toDateSelectedDate)
                    : getDateTimeJsonPersian1(toDateSelectedDate);
            } else if (setting.toDate) {
                var fromDateSetting = getSetting2($fromDateElement),
                    fromDateSelectedDate = fromDateSetting.selectedDate;
                disableBeforeDateTimeJson = !fromDateSelectedDate
                    ? undefined
                    : setting.isGregorian
                    ? getDateTimeJson1(fromDateSelectedDate)
                    : getDateTimeJsonPersian1(fromDateSelectedDate);
            }
        }
        counter = 1;
        var yearStart = yearToStart
            ? yearToStart
            : todayDateTimeJson.year - setting.yearOffset;
        var yearEnd = yearToStart
            ? yearToStart + setting.yearOffset * 2
            : todayDateTimeJson.year + setting.yearOffset;
        for (var i = yearStart; i < yearEnd; i++) {
            if (setting.disableBeforeToday && i < todayDateTimeJson.year)
                continue;
            if (setting.disableAfterToday && i > todayDateTimeJson.year)
                continue;
            if (
                disableBeforeDateTimeJson != undefined &&
                disableBeforeDateTimeJson.year != undefined &&
                i < disableBeforeDateTimeJson.year
            )
                continue;
            if (
                disableAfterDateTimeJson != undefined &&
                disableAfterDateTimeJson.year != undefined &&
                i > disableAfterDateTimeJson.year
            )
                continue;
            var currentYearDateTimeJson = getDateTimeJson2(
                    convertToNumber2(
                        i,
                        selectedDateTimeToShowJson.month,
                        getDaysInMonthPersian(
                            i,
                            selectedDateTimeToShowJson.month
                        )
                ...
                yearText = setting.englishNumber
                    ? i.toString()
                    : toPersianNumber(i),
                yearDateNumber = convertToNumber2(
                    i,
                    selectedDateTimeToShowJson.month,
                    1
                );
            if (
                disableBeforeDateTimeJson != undefined &&
                disableBeforeDateTimeJson.year != undefined &&
                currentYearDateTimeJson.year < disableBeforeDateTimeJson.year
            )
                currentYearDisabledAttr = "disabled";
            if (
                disableAfterDateTimeJson != undefined &&
                disableAfterDateTimeJson.year != undefined &&
                currentYearDateTimeJson.year > disableAfterDateTimeJson.year
            )
                currentYearDisabledAttr = "disabled";
            if (
                setting.disableBeforeToday &&
                currentYearDateTimeJson.year < todayDateTimeJson.year
            )
                currentYearDisabledAttr = "disabled";
            if (
                setting.disableAfterToday &&
                currentYearDateTimeJson.year > todayDateTimeJson.year
            )
                currentYearDisabledAttr = "disabled";
...
<td class="text-center" ${
                selectedDateTimeToShowJson.year == i ? "selected-year" : ""
            }>
...
`;
        ...
        html = html.replace(/{{yearsToSelectHtml}}/gim, yearsToSelectHtml);
        return {
            yearStart,
            yearEnd,
            html,
    ...
    function getDateTimePickerHtml(setting) {
        ...
        html = html.replace(
            /{{rtlCssClass}}/gim,
            setting.isGregorian ? "" : "rtl"
        );
        html = html.replace(
            /{{selectedDateStringAttribute}}/gim,
            setting.inLine ? "" : "hidden"
        );
        html = html.replace(
            /{{hourText}}/gim,
            setting.isGregorian ? hourText : hourTextPersian
        );
        html = html.replace(
            /{{minuteText}}/gim,
            setting.isGregorian ? minuteText : minuteTextPersian
        );
        html = html.replace(
            /{{secondText}}/gim,
            setting.isGregorian ? secondText : secondTextPersian
        );
        html = html.replace(
            /{{goTodayText}}/gim,
            setting.isGregorian ? goTodayText : goTodayTextPersian
        );
        html = html.replace(
            /{{timePickerAttribute}}/gim,
            setting.enableTimePicker ? "" : "hidden"
        );

        var selectedDateString = "",
            todayDateString = "",
            todayDateTimeJson = {}, // year, month, day, hour, minute, second
            rangeSelectorStartDate =
                !setting.rangeSelector || !setting.rangeSelectorStartDate
                    ? undefined
                    : getClonedDate(setting.rangeSelectorStartDate),
            rangeSelectorEndDate =
                !setting.rangeSelector || !setting.rangeSelectorEndDate
                    ? undefined
                    : getClonedDate(setting.rangeSelectorEndDate),
        ...
        if (setting.isGregorian) {
            ...
            rangeSelectorStartDateJson =
                rangeSelectorStartDate != undefined
                    ? getDateTimeJson1(rangeSelectorStartDate)
                    : undefined;
            rangeSelectorEndDateJson =
                rangeSelectorEndDate != undefined
                    ? getDateTimeJson1(rangeSelectorEndDate)
                    : undefined;
            selectedDateTimeJson =
            ...
            rangeSelectorStartDateJson =
                rangeSelectorStartDate != undefined
                    ? getDateTimeJsonPersian1(rangeSelectorStartDate)
                    : undefined;
            rangeSelectorEndDateJson =
                rangeSelectorEndDate != undefined
                    ? getDateTimeJsonPersian1(rangeSelectorEndDate)
                    : undefined;
            selectedDateTimeJson =
                setting.selectedDate == undefined
                    ? todayDateTimeJson
        ...
        if (
            setting.rangeSelector &&
            rangeSelectorStartDateJson != undefined &&
            rangeSelectorEndDateJson != undefined
        ) {
            selectedDateString = `${getWeekDayName(
                rangeSelectorStartDateJson.dayOfWeek,
                setting.isGregorian
            )}، ${rangeSelectorStartDateJson.day} ${getMonthName(
                rangeSelectorStartDateJson.month - 1,
                setting.isGregorian
            )} ${rangeSelectorStartDateJson.year} - 
                ${getWeekDayName(
                    rangeSelectorEndDateJson.dayOfWeek,
                    setting.isGregorian
                )}، ${rangeSelectorEndDateJson.day} ${getMonthName(
                rangeSelectorEndDateJson.month - 1,
                setting.isGregorian
            )} ${rangeSelectorEndDateJson.year}`;
        } else
            selectedDateString = `${getWeekDayName(
                selectedDateTimeJson.dayOfWeek,
                setting.isGregorian
            )}، ${selectedDateTimeJson.day} ${getMonthName(
                selectedDateTimeJson.month - 1,
                setting.isGregorian
            )} ${selectedDateTimeJson.year}`;
        todayDateString = `${setting.isGregorian ? "Today," : "امروز،"} ${
            todayDateTimeJson.day
        } ${getMonthName(todayDateTimeJson.month - 1, setting.isGregorian)} ${
            todayDateTimeJson.year
        }`;
        ...
        if (
            disableAfterDateTimeJson != undefined &&
            disableAfterDateTimeJson.year <= selectedDateTimeToShowJson.year &&
            disableAfterDateTimeJson.month < selectedDateTimeToShowJson.month
        )
            selectedDateToShow = setting.isGregorian
                ? new Date(
                      disableAfterDateTimeJson.year,
                      disableAfterDateTimeJson.month - 1,
                      1
                  )
                : getDateTime1(
                      disableAfterDateTimeJson.year,
                      disableAfterDateTimeJson.month,
                      disableAfterDateTimeJson.day
                  );

        if (
            disableBeforeDateTimeJson != undefined &&
            disableBeforeDateTimeJson.year >= selectedDateTimeToShowJson.year &&
            disableBeforeDateTimeJson.month > selectedDateTimeToShowJson.month
        )
            selectedDateToShow = setting.isGregorian
                ? new Date(
                      disableBeforeDateTimeJson.year,
                      disableBeforeDateTimeJson.month - 1,
                      1
                  )
                : getDateTime1(
                      disableBeforeDateTimeJson.year,
                      disableBeforeDateTimeJson.month,
                      disableBeforeDateTimeJson.day
                  );

        var monthsTdHtml = "",
            numberOfNextMonths =
                setting.monthsToShow[1] <= 0 ? 0 : setting.monthsToShow[1],
            numberOfPrevMonths =
                setting.monthsToShow[0] <= 0 ? 0 : setting.monthsToShow[0];
        numberOfPrevMonths *= -1;
        for (var i1 = numberOfPrevMonths; i1 < 0; i1++) {
            setting.selectedDateToShow = addMonthToDateTime(
                getClonedDate(selectedDateToShow),
                i1
            );
        ...
        var totalMonthNumberToShow =
                Math.abs(numberOfPrevMonths) + 1 + numberOfNextMonths,
            monthTdStyle =
                totalMonthNumberToShow > 1
                    ? "width: " +
                      (100 / totalMonthNumberToShow).toString() +
                      "%;"
                    : "";

        monthsTdHtml = monthsTdHtml.replace(
            /{{monthTdStyle}}/gim,
            monthTdStyle
        );

        html = html.replace(/{{selectedDateString}}/gim, selectedDateString);
        html = html.replace(/{{todayDateString}}/gim, todayDateString);
        html = html.replace(/{{hour}}/gim, selectedDateTimeToShowJson.hour);
        html = html.replace(/{{minute}}/gim, selectedDateTimeToShowJson.minute);
        html = html.replace(/{{second}}/gim, selectedDateTimeToShowJson.second);
        html = html.replace(/{{monthsTdHtml}}/gim, monthsTdHtml);
    ...
    function getDateTimePickerMonthHtml1(setting, isNextMonth, isPrevMonth) {
        ...
        html = html.replace(
            /{{monthNameAttribute}}/gim,
            !isNextOrPrevMonth ? "hidden" : ""
        );
        html = html.replace(
            /{{theadSelectDateButtonTrAttribute}}/gim,
            setting.inLine || !isNextOrPrevMonth ? "" : "hidden"
        );
        html = html.replace(
            /{{weekDayShortName1CssClass}}/gim,
            setting.isGregorian ? "text-danger" : ""
        );
        html = html.replace(
            /{{weekDayShortName7CssClass}}/gim,
            !setting.isGregorian ? "text-danger" : ""
        );
        html = html.replace(
            /{{previousYearText}}/gim,
            setting.isGregorian ? previousYearText : previousYearTextPersian
        );
        html = html.replace(
            /{{previousMonthText}}/gim,
            setting.isGregorian ? previousMonthText : previousMonthTextPersian
        );
        html = html.replace(
            /{{nextMonthText}}/gim,
            setting.isGregorian ? nextMonthText : nextMonthTextPersian
        );
        html = html.replace(
            /{{nextYearText}}/gim,
            setting.isGregorian ? nextYearText : nextYearTextPersian
        );
        html = html.replace(
            /{{monthName1}}/gim,
            getMonthName(0, setting.isGregorian)
        );
        html = html.replace(
            /{{monthName2}}/gim,
            getMonthName(1, setting.isGregorian)
        );
        html = html.replace(
            /{{monthName3}}/gim,
            getMonthName(2, setting.isGregorian)
        );
        html = html.replace(
            /{{monthName4}}/gim,
            getMonthName(3, setting.isGregorian)
        );
        html = html.replace(
            /{{monthName5}}/gim,
            getMonthName(4, setting.isGregorian)
        );
        html = html.replace(
            /{{monthName6}}/gim,
            getMonthName(5, setting.isGregorian)
        );
        html = html.replace(
            /{{monthName7}}/gim,
            getMonthName(6, setting.isGregorian)
        );
        html = html.replace(
            /{{monthName8}}/gim,
            getMonthName(7, setting.isGregorian)
        );
        html = html.replace(
            /{{monthName9}}/gim,
            getMonthName(8, setting.isGregorian)
        );
        html = html.replace(
            /{{monthName10}}/gim,
            getMonthName(9, setting.isGregorian)
        );
        html = html.replace(
            /{{monthName11}}/gim,
            getMonthName(10, setting.isGregorian)
        );
        html = html.replace(
            /{{monthName12}}/gim,
            getMonthName(11, setting.isGregorian)
        );
        html = html.replace(
            /{{weekDayShortName1}}/gim,
            getWeekDayShortName(0, setting.isGregorian)
        );
        html = html.replace(
            /{{weekDayShortName2}}/gim,
            getWeekDayShortName(1, setting.isGregorian)
        );
        html = html.replace(
            /{{weekDayShortName3}}/gim,
            getWeekDayShortName(2, setting.isGregorian)
        );
        html = html.replace(
            /{{weekDayShortName4}}/gim,
            getWeekDayShortName(3, setting.isGregorian)
        );
        html = html.replace(
            /{{weekDayShortName5}}/gim,
            getWeekDayShortName(4, setting.isGregorian)
        );
        html = html.replace(
            /{{weekDayShortName6}}/gim,
            getWeekDayShortName(5, setting.isGregorian)
        );
        html = html.replace(
            /{{weekDayShortName7}}/gim,
            getWeekDayShortName(6, setting.isGregorian)
        );
        ...
        if (setting.isGregorian) {
            ...
            disableAfterDateTimeJson = !setting.disableAfterDate
                ? undefined
                : getDateTimeJson1(setting.disableAfterDate);
            firstWeekDayNumber = new Date(
                dateTimeToShowJson.year,
                dateTimeToShowJson.month - 1,
                1
            ).getDay();
            selectedDateNumber = !selectedDateTime
                ? 0
                : convertToNumber1(getDateTimeJson1(selectedDateTime));
            numberOfDaysInCurrentMonth = getDaysInMonth(
                dateTimeToShowJson.year,
                dateTimeToShowJson.month - 1
            );
            numberOfDaysInPreviousMonth = getDaysInMonth(
                dateTimeToShowJson.year,
                dateTimeToShowJson.month - 2
            );
            previousMonthDateNumber = convertToNumber1(
                getDateTimeJson1(
                    getLastDayDateOfPreviousMonth(selectedDateToShowTemp, true)
                )
            );
            nextMonthDateNumber = convertToNumber1(
                getDateTimeJson1(
                    getFirstDayDateOfNextMonth(selectedDateToShowTemp, true)
                )
            );
            selectedDateToShowTemp = getClonedDate(selectedDateToShow);
            previousYearDateNumber = convertToNumber1(
                getDateTimeJson1(
                    new Date(
                        selectedDateToShowTemp.setFullYear(
                            selectedDateToShowTemp.getFullYear() - 1
                        )
                    )
                )
            );
            selectedDateToShowTemp = getClonedDate(selectedDateToShow);
            nextYearDateNumber = convertToNumber1(
                getDateTimeJson1(
                    new Date(
                        selectedDateToShowTemp.setFullYear(
                            selectedDateToShowTemp.getFullYear() + 1
            ...
            selectedDateToShowTemp = getClonedDate(selectedDateToShow);
            rangeSelectorStartDateNumber =
                !setting.rangeSelector || !rangeSelectorStartDate
                    ? 0
                    : convertToNumber3(rangeSelectorStartDate);
            rangeSelectorEndDateNumber =
                !setting.rangeSelector || !rangeSelectorEndDate
                    ? 0
                    : convertToNumber3(rangeSelectorEndDate);
            for (i = 1; i <= 12; i++) {
                monthsDateNumberAndAttr["month" + i.toString() + "DateNumber"] =
                    convertToNumber1(
                        getDateTimeJson1(
                            new Date(selectedDateToShowTemp.setMonth(i - 1))
                        )
                    );
                selectedDateToShowTemp = getClonedDate(selectedDateToShow);
            }
            for (i = 0; i < setting.holiDays.length; i++) {
                holidaysDateNumbers.push(
                    convertToNumber1(getDateTimeJson1(setting.holiDays[i]))
                );
            }
            for (i = 0; i < setting.disabledDates.length; i++) {
                disabledDatesNumber.push(
                    convertToNumber1(getDateTimeJson1(setting.disabledDates[i]))
                );
            }
            for (i = 0; i < setting.specialDates.length; i++) {
                specialDatesNumber.push(
                    convertToNumber1(getDateTimeJson1(setting.specialDates[i]))
                );
            }
        } else {
            dateTimeToShowJson = getDateTimeJsonPersian1(
                selectedDateToShowTemp
            );
            todayDateTimeJson = getDateTimeJsonPersian1(new Date());
            disableBeforeDateTimeJson = !setting.disableBeforeDate
                ? undefined
            ...
            disableAfterDateTimeJson = !setting.disableAfterDate
                ? undefined
                : getDateTimeJsonPersian1(setting.disableAfterDate);
            firstWeekDayNumber = getDateTimeJsonPersian2(
                dateTimeToShowJson.year,
                dateTimeToShowJson.month,
                1,
                0,
                0,
                0
            ...
            selectedDateNumber = !selectedDateTime
                ? 0
                : convertToNumber1(getDateTimeJsonPersian1(selectedDateTime));
            numberOfDaysInCurrentMonth = getDaysInMonthPersian(
                dateTimeToShowJson.year,
                dateTimeToShowJson.month
            );
            numberOfDaysInPreviousMonth = getDaysInMonthPersian(
                dateTimeToShowJson.year - 1,
                dateTimeToShowJson.month - 1
            );
            previousMonthDateNumber = convertToNumber1(
                getDateTimeJsonPersian1(
                    getLastDayDateOfPreviousMonth(selectedDateToShowTemp, false)
                )
            );
            selectedDateToShowTemp = getClonedDate(selectedDateToShow);
            nextMonthDateNumber = convertToNumber1(
                getDateTimeJsonPersian1(
                    getFirstDayDateOfNextMonth(selectedDateToShowTemp, false)
                )
            );
            selectedDateToShowTemp = getClonedDate(selectedDateToShow);
            previousYearDateNumber = convertToNumber2(
                dateTimeToShowJson.year - 1,
                dateTimeToShowJson.month,
                dateTimeToShowJson.day
            );
            nextYearDateNumber = convertToNumber2(
                dateTimeToShowJson.year + 1,
                dateTimeToShowJson.month,
                dateTimeToShowJson.day
            );
            ...
            for (i = 1; i <= 12; i++) {
                monthsDateNumberAndAttr["month" + i.toString() + "DateNumber"] =
                    convertToNumber2(
                        dateTimeToShowJson.year,
                        i,
                        getDaysInMonthPersian(dateTimeToShowJson.year, i)
                    );
                selectedDateToShowTemp = getClonedDate(selectedDateToShow);
            }
            for (i = 0; i < setting.holiDays.length; i++) {
                holidaysDateNumbers.push(
                    convertToNumber1(
                        getDateTimeJsonPersian1(setting.holiDays[i])
                    )
                );
            }
            for (i = 0; i < setting.disabledDates.length; i++) {
                disabledDatesNumber.push(
                    convertToNumber1(
                        getDateTimeJsonPersian1(setting.disabledDates[i])
                    )
                );
            }
            for (i = 0; i < setting.specialDates.length; i++) {
                specialDatesNumber.push(
                    convertToNumber1(
                        getDateTimeJsonPersian1(setting.specialDates[i])
                    )
                );
            }
        }
        ...
        selectedYear = setting.englishNumber
            ? dateTimeToShowJson.year
            : toPersianNumber(dateTimeToShowJson.year);
        disableBeforeDateTimeNumber = !disableBeforeDateTimeJson
            ? undefined
            : convertToNumber1(disableBeforeDateTimeJson);
        disableAfterDateTimeNumber = !disableAfterDateTimeJson
            ? undefined
            : convertToNumber1(disableAfterDateTimeJson);
        currentMonthInfo =
            getMonthName(dateTimeToShowJson.month - 1, setting.isGregorian) +
            " " +
            dateTimeToShowJson.year.toString();
        if (!setting.englishNumber)
            currentMonthInfo = toPersianNumber(currentMonthInfo);
        selectedMonthName = getMonthName(
            dateTimeToShowJson.month - 1,
            setting.isGregorian
        );
        ...
        if (firstWeekDayNumber != 6) {
            ...
            var previousMonthDateTimeJson = addMonthToDateTimeJson(
                dateTimeToShowJson,
                -1,
                setting.isGregorian
            );
            for (
                i = numberOfDaysInPreviousMonth - firstWeekDayNumber;
                i <= numberOfDaysInPreviousMonth;
                i++
            ) {
                currentDateNumber = convertToNumber2(
                    previousMonthDateTimeJson.year,
                    previousMonthDateTimeJson.month,
                    i
                );
                dayNumberInString = setting.englishNumber
                    ? zeroPad(i)
                    : toPersianNumber(zeroPad(i));
                $td = $("<td data-nm />")
                    .attr("data-number", currentDateNumber)
                    .html(dayNumberInString);
                if (setting.rangeSelector) {
                    if (
                        currentDateNumber == rangeSelectorStartDateNumber ||
                        currentDateNumber == rangeSelectorEndDateNumber
                    )
        ...
        for (i = 1; i <= numberOfDaysInCurrentMonth; i++) {
            ...
            if (currentDateNumber == todayDateNumber) {
                ...
                if (!dayOfWeek)
                    dayOfWeek = getWeekDayName(
                        tdNumber - 1 < 0 ? 0 : tdNumber - 1,
                        setting.isGregorian
                    );
            }
            ...
            if (setting.disableBeforeToday) {
                ...
                for (j = 1; j <= 12; j++) {
                    if (
                        monthsDateNumberAndAttr[
                            "month" + j.toString() + "DateNumber"
                        ] < todayDateNumber
                    )
                        monthsDateNumberAndAttr[
                            "selectMonth" + j.toString() + "ButtonCssClass"
                        ] = "disabled";
                }
            }
            ...
            if (
                ...
                setting.disabledDays.indexOf(tdNumber) >= 0
            ) {
                $td.attr("disabled", "");
            }
            // \\
        ...
        // روزهای ماه بعد
        var nextMonthDateTimeJson = addMonthToDateTimeJson(
            dateTimeToShowJson,
            1,
            setting.isGregorian
        );
        for (i = 1; i <= 42 - cellNumber; i++) {
            dayNumberInString = setting.englishNumber
                ...
                : toPersianNumber(zeroPad(i));
            currentDateNumber = convertToNumber2(
                nextMonthDateTimeJson.year,
                nextMonthDateTimeJson.month,
                i
            );
            $td = $("<td data-nm />")
                .attr("data-number", currentDateNumber)
        ...
        html = html.replace(/{{currentMonthInfo}}/gim, currentMonthInfo);
        html = html.replace(/{{selectedYear}}/gim, selectedYear);
        html = html.replace(/{{selectedMonthName}}/gim, selectedMonthName);
        html = html.replace(/{{daysHtml}}/gim, daysHtml);
        html = html.replace(
            /{{previousYearButtonDisabledAttribute}}/gim,
            previousYearButtonDisabledAttribute
        );
        html = html.replace(
            /{{previousYearButtonDateNumber}}/gim,
            previousYearDateNumber
        );
        html = html.replace(
            /{{previousMonthButtonDisabledAttribute}}/gim,
            previousMonthButtonDisabledAttribute
        );
        html = html.replace(
            /{{previousMonthButtonDateNumber}}/gim,
            previousMonthDateNumber
        );
        html = html.replace(
            /{{selectYearButtonDisabledAttribute}}/gim,
            selectYearButtonDisabledAttribute
        );
        html = html.replace(
            /{{nextMonthButtonDisabledAttribute}}/gim,
            nextMonthButtonDisabledAttribute
        );
        html = html.replace(
            /{{nextMonthButtonDateNumber}}/gim,
            nextMonthDateNumber
        );
        html = html.replace(
            /{{nextYearButtonDisabledAttribute}}/gim,
            nextYearButtonDisabledAttribute
        );
        html = html.replace(
            /{{nextYearButtonDateNumber}}/gim,
            nextYearDateNumber
        );
        html = html.replace(
            /{{dropDownMenuMonth1DateNumber}}/gim,
            monthsDateNumberAndAttr.month1DateNumber
        );
        html = html.replace(
            /{{dropDownMenuMonth2DateNumber}}/gim,
            monthsDateNumberAndAttr.month2DateNumber
        );
        html = html.replace(
            /{{dropDownMenuMonth3DateNumber}}/gim,
            monthsDateNumberAndAttr.month3DateNumber
        );
        html = html.replace(
            /{{dropDownMenuMonth4DateNumber}}/gim,
            monthsDateNumberAndAttr.month4DateNumber
        );
        html = html.replace(
            /{{dropDownMenuMonth5DateNumber}}/gim,
            monthsDateNumberAndAttr.month5DateNumber
        );
        html = html.replace(
            /{{dropDownMenuMonth6DateNumber}}/gim,
            monthsDateNumberAndAttr.month6DateNumber
        );
        html = html.replace(
            /{{dropDownMenuMonth7DateNumber}}/gim,
            monthsDateNumberAndAttr.month7DateNumber
        );
        html = html.replace(
            /{{dropDownMenuMonth8DateNumber}}/gim,
            monthsDateNumberAndAttr.month8DateNumber
        );
        html = html.replace(
            /{{dropDownMenuMonth9DateNumber}}/gim,
            monthsDateNumberAndAttr.month9DateNumber
        );
        html = html.replace(
            /{{dropDownMenuMonth10DateNumber}}/gim,
            monthsDateNumberAndAttr.month10DateNumber
        );
        html = html.replace(
            /{{dropDownMenuMonth11DateNumber}}/gim,
            monthsDateNumberAndAttr.month11DateNumber
        );
        html = html.replace(
            /{{dropDownMenuMonth12DateNumber}}/gim,
            monthsDateNumberAndAttr.month12DateNumber
        );
        html = html.replace(
            /{{selectMonth1ButtonCssClass}}/gim,
            monthsDateNumberAndAttr.selectMonth1ButtonCssClass
        );
        html = html.replace(
            /{{selectMonth2ButtonCssClass}}/gim,
            monthsDateNumberAndAttr.selectMonth2ButtonCssClass
        );
        html = html.replace(
            /{{selectMonth3ButtonCssClass}}/gim,
            monthsDateNumberAndAttr.selectMonth3ButtonCssClass
        );
        html = html.replace(
            /{{selectMonth4ButtonCssClass}}/gim,
            monthsDateNumberAndAttr.selectMonth4ButtonCssClass
        );
        html = html.replace(
            /{{selectMonth5ButtonCssClass}}/gim,
            monthsDateNumberAndAttr.selectMonth5ButtonCssClass
        );
        html = html.replace(
            /{{selectMonth6ButtonCssClass}}/gim,
            monthsDateNumberAndAttr.selectMonth6ButtonCssClass
        );
        html = html.replace(
            /{{selectMonth7ButtonCssClass}}/gim,
            monthsDateNumberAndAttr.selectMonth7ButtonCssClass
        );
        html = html.replace(
            /{{selectMonth8ButtonCssClass}}/gim,
            monthsDateNumberAndAttr.selectMonth8ButtonCssClass
        );
        html = html.replace(
            /{{selectMonth9ButtonCssClass}}/gim,
            monthsDateNumberAndAttr.selectMonth9ButtonCssClass
        );
        html = html.replace(
            /{{selectMonth10ButtonCssClass}}/gim,
            monthsDateNumberAndAttr.selectMonth10ButtonCssClass
        );
        html = html.replace(
            /{{selectMonth11ButtonCssClass}}/gim,
            monthsDateNumberAndAttr.selectMonth11ButtonCssClass
        );
        html = html.replace(
            /{{selectMonth12ButtonCssClass}}/gim,
            monthsDateNumberAndAttr.selectMonth12ButtonCssClass
        );
    ...
    function unbindEvents() {
        ...
        $(document).off(
            "mouseenter",
            mdDatePickerContainerSelector + " [data-day]"
        );
        $(document).off(
            "click",
            mdDatePickerContainerSelector + " [data-changedatebutton]"
        );
        $(document).off(
            "blur",
            mdDatePickerContainerSelector + " input[data-clock]"
        );
        $(document).off(
            "blur",
            mdDatePickerContainerSelector + " input[data-clock]"
        ...
        $(document).off(
            "click",
            mdDatePickerContainerSelector + " [select-year-button]"
        );
        $(document).off("click", "[data-yearrangebuttonchange]");
        $(document).off(
            "click",
            mdDatePickerContainerSelector + " [data-go-today]"
        );
    ...
    $(document).on(
        ...
        function (e) {
            var $this = $(this),
                ...
                selectedDateToShowJson =
                    selectedDateToShow == undefined
                        ? undefined
                        : getDateTimeJson1(selectedDateToShow);
            if (disabled) {
            ...
            selectedDateToShow = getDateTime4(
                dateNumber,
                selectedDateToShow,
                setting
            );
            if (setting.rangeSelector) {
                ...
                if (
                    ...
                    $this
                        .parents("table:last")
                        .find(
                            "td.selected-range-days-start-end,td.selected-range-days"
                        )
            ...
            if (!setting.inLine) {
                ...
                if (setting.fromDate && $toDateDayElement.length > 0) {
                    updateCalendarHtml1(
                        $toDateDayElement,
                        getSetting1($toDateDayElement)
                    );
                } else if (setting.toDate && $fromDateDayElement.length > 0) {
                    updateCalendarHtml1(
                        $fromDateDayElement,
                        getSetting1($fromDateDayElement)
                    );
                }
    ...
    $(document).on(
        ...
        function () {
            ...
            var rangeSelectorStartDate = !setting.rangeSelectorStartDate
                    ? undefined
                    : getClonedDate(setting.rangeSelectorStartDate),
                rangeSelectorEndDate = !setting.rangeSelectorEndDate
                    ? undefined
            ...
            if (setting.isGregorian) {
                rangeSelectorStartDateNumber = !rangeSelectorStartDate
                    ? 0
                    : convertToNumber3(rangeSelectorStartDate);
                rangeSelectorEndDateNumber = !rangeSelectorEndDate
                    ? 0
                    : convertToNumber3(rangeSelectorEndDate);
            } else {
                rangeSelectorStartDateNumber = !rangeSelectorStartDate
            ...
            if (
                rangeSelectorStartDateNumber > 0 &&
                dateNumber > rangeSelectorStartDateNumber
            ) {
                for (
                    var i1 = rangeSelectorStartDateNumber;
                    i1 <= dateNumber;
                    i1++
                ) {
                    $allTdDays
                        .filter(
                            '[data-number="' +
                                i1.toString() +
                                '"]:not(.selected-range-days-start-end)'
                        )
                        .addClass("selected-range-days");
                }
            } else if (
                rangeSelectorEndDateNumber > 0 &&
                dateNumber < rangeSelectorEndDateNumber
            ) {
                for (
                    var i2 = dateNumber;
                    i2 <= rangeSelectorEndDateNumber;
                    i2++
                ) {
                    $allTdDays
                        .filter(
    ...
    $(document).on(
        ...
        function () {
            var $this = $(this),
                $thisContainer = $this.parents(
                    mdDatePickerContainerSelector + ":first"
                ),
                $hour = $thisContainer.find(
                    'input[type="text"][data-clock="hour"]'
                ),
                $minute = $thisContainer.find(
                    'input[type="text"][data-clock="minute"]'
                ),
                $second = $thisContainer.find(
                    'input[type="text"][data-clock="second"]'
                ),
            ...
            hour = !isNumber(hour)
                ? setting.selectedDateToShow.getHours()
                : hour;
            minute = !isNumber(minute)
                ? setting.selectedDateToShow.getMinutes()
                : minute;
            second = !isNumber(second)
                ? setting.selectedDateToShow.getSeconds()
                : second;

            setting.selectedDateToShow = new Date(
                setting.selectedDateToShow.setHours(hour)
            );
            setting.selectedDateToShow = new Date(
                setting.selectedDateToShow.setMinutes(minute)
            );
            setting.selectedDateToShow = new Date(
                setting.selectedDateToShow.setSeconds(second)
            );

            if (setting.selectedDate == undefined)
                setting.selectedDate = new Date();
            setting.selectedDate = new Date(
                setting.selectedDate.setHours(hour)
            );
            setting.selectedDate = new Date(
                setting.selectedDate.setMinutes(minute)
            );
            setting.selectedDate = new Date(
                setting.selectedDate.setSeconds(second)
            );
    ...
    $(document).on(
        ...
        function () {
            var $this = $(this),
                ...
                $mdDatePickerContainerSelector = $this.parents(
                    mdDatePickerContainerSelector + ":first"
                ),
                $dateTimePickerYearsToSelectContainer =
                    $mdDatePickerContainerSelector.find(
                        '[data-name="dateTimePickerYearsToSelectContainer"]'
                    );
            popoverHeaderHtml = popoverHeaderHtml.replace(
                /{{rtlCssClass}}/gim,
                setting.isGregorian ? "" : "rtl"
            );
            popoverHeaderHtml = popoverHeaderHtml.replace(
                ...
                setting.isGregorian || setting.englishNumber
                    ? yearsRangeText
                    : toPersianNumber(yearsRangeText)
            );
            popoverHeaderHtml = popoverHeaderHtml.replace(
                /{{previousText}}/gim,
                setting.isGregorian ? previousText : previousTextPersian
            );
            popoverHeaderHtml = popoverHeaderHtml.replace(
                /{{nextText}}/gim,
                setting.isGregorian ? nextText : nextTextPersian
            );
            popoverHeaderHtml = popoverHeaderHtml.replace(
                /{{latestPreviousYear}}/gim,
                yearsToSelectObject.yearStart > yearsToSelectObject.yearEnd
                    ? yearsToSelectObject.yearEnd
                    : yearsToSelectObject.yearStart
            );
            popoverHeaderHtml = popoverHeaderHtml.replace(
                /{{latestNextYear}}/gim,
                yearsToSelectObject.yearStart > yearsToSelectObject.yearEnd
            ...
            $dateTimePickerYearsToSelectContainer.html(
                dateTimePickerYearsToSelectHtml
            );
    ...
    $(document).on("click", "[data-yearrangebuttonchange]", function () {
        var $this = $(this),
            ...
            yearsToSelectObject = getYearsToSelectHtml(
                setting,
                isNext ? yearStart : yearStart - setting.yearOffset * 2
            ),
            yearsRangeText = ` ${yearsToSelectObject.yearStart} - ${
                yearsToSelectObject.yearEnd - 1
            } `,
            popoverHeaderHtml = popoverHeaderSelectYearHtmlTemplate,
            dateTimePickerYearsToSelectHtml = yearsToSelectObject.html;
        popoverHeaderHtml = popoverHeaderHtml.replace(
        ...
        setPopoverHeaderHtml($this, setting.inLine, popoverHeaderHtml);
        $(mdDatePickerContainerSelector)
            .find('[data-name="dateTimePickerYearsToSelectContainer"]')
            .html(dateTimePickerYearsToSelectHtml);
    });
    ...
    $("html").on("click", function (e) {
        ...
        if (
            $popoverDescriber.length >= 1 ||
            isWithinMdModal($target) ||
            isCalendarOpen($target)
        )
            return;
        hidePopover($(mdDatePickerElementSelector));
    });
    ...
    var methods = {
            ...
            return this.each(function () {
                ...
                if (setting.targetDateSelector) {
                    ...
                    if (targetValue) {
                        ...
                        setting.selectedDateToShow = getClonedDate(
                    ...
                    if (textValue) {
                        setting.selectedDate = parseDateTime(
                            textValue,
                            setting
                        );
                        setting.selectedDateToShow = getClonedDate(
                ...
                if (setting.toDate && setting.fromDate)
                    throw new Error(
                        `MdPersianDateTimePicker => You can not set true 'toDate' and 'fromDate' together`
                    );
                if (!setting.groupId && (setting.toDate || setting.fromDate))
                    throw new Error(
                        `MdPersianDateTimePicker => When you set 'toDate' or 'fromDate' true, you have to set 'groupId'`
                    );
                ...
                if (
                    setting.rangeSelector &&
                    setting.selectedRangeDate != undefined
                ) {
                    ...
                    $this
                        ...
                        .on(setting.trigger, function () {
                            ...
                            setTimeout(function () {
                                ...
                                var calendarHtml =
                                    getDateTimePickerHtml(setting);
                                setPopoverHeaderHtml(
                                    $this,
                                    setting.inLine,
                                    $(calendarHtml)
                                        .find("[data-selecteddatestring]")
                                        .text()
                                        .trim()
                                );
                                getPopover($this)
                                    .find(
                                        '[data-name="mds-datetimepicker-body"]'
                                    )
                                    .html(calendarHtml);
                                $this.popover("update");
                                triggerStart = false;
                    ...
                    $this.on("click", function () {
                        ...
                        setting.selectedDateToShow =
                            setting.selectedDate != undefined
                                ...
                                : new Date();
                        var calendarHtml = getDateTimePickerHtml(setting);
                        $(mdDatePickerElementSelector)
                            .find('[data-name="mds-datetimepicker-body"]')
                            .html(calendarHtml);
                        $(mdDatePickerElementSelector)
                            .find("[data-buttonselector]")
                            .attr("data-buttonselector", uniqueId);
                        $(mdDatePickerElementSelector).modal("show");
                    });
                }
                $(document).on(
                    "change",
                    setting.targetTextSelector,
                    function () {
                        ...
                        try {
                            if (!setting.rangeSelector)
                                $this.MdPersianDateTimePicker(
                                    "setDate",
                                    parseDateTime(value1, setting)
                                );
                            else {
                                let dateValues = value1.split(" - ");
                                $this.MdPersianDateTimePicker(
                                    "setDateRange",
                                    parseDateTime(dateValues[0], setting),
                                    parseDateTime(dateValues[1], setting)
                                );
                            }
                        } catch (e) {
                            setSelectedData(setting);
        ...
        getText: function () {
            ...
            this.each(function () {
                textArray.push(
                    getSelectedDateTimeTextFormatted(getSetting2($(this)))
                );
            });
        ...
        getDateRange: function () {
            ...
            this.each(function () {
                ...
                if ((!setting.toDate && !setting.fromDate) || !setting.groupId)
                    return [];
                var fromDateSetting = getSetting2(
                        $(
                            "[" +
                                mdDatePickerGroupIdAttribute +
                                '="' +
                ...
                dateRangeArray.push([
                    fromDateSetting.selectedDate,
                    toDateSetting.selectedDate,
                ]);
            });
        ...
        setDate: function (dateTimeObject) {
            if (dateTimeObject == undefined)
                throw new Error(
                    "MdPersianDateTimePicker => setDate => مقدار ورودی نا معتبر است"
                );
            return this.each(function () {
                var $this = $(this),
        ...
        setOption: function (name, value) {
            if (!name)
                throw new Error(
                    "MdPersianDateTimePicker => setOption => name parameter مقدار ورودی نا معتبر است"
                );
            return this.each(function () {
                var $this = $(this),
                    setting = getSetting2($this);
        ...
        setDateRange: function (startDateTimeObject, endDateTimeObject) {
            if (
                startDateTimeObject == undefined ||
                endDateTimeObject == undefined
            )
                throw new Error(
                    "MdPersianDateTimePicker => setDateRange => مقدار ورودی نا معتبر است"
                );
            if (
                convertToNumber4(startDateTimeObject) >
                convertToNumber4(endDateTimeObject)
            )
                throw new Error(
                    "MdPersianDateTimePicker => setDateRange => مقدار ورودی نا معتبر است, تاریخ شروع باید بزرگتر از تاریخ پایان باشد"
                );
            return this.each(function () {
                var $this = $(this),
                    setting = getSetting2($this);
                if (setting.rangeSelector) {
                    ...
                    (setting.fromDate || setting.toDate) &&
                    setting.groupId
                ) {
                    var $toDateElement = $(
                            "[" +
        ...
        setDatePersian: function (dateTimeObjectJson) {
            if (dateTimeObjectJson == undefined)
                throw new Error(
                    "MdPersianDateTimePicker => setDatePersian => ورودی باید از نوه جی سان با حداقل پراپرتی های year, month, day باشد"
                );
            dateTimeObjectJson.hour = !dateTimeObjectJson.hour
                ? 0
                : dateTimeObjectJson.hour;
            dateTimeObjectJson.minute = !dateTimeObjectJson.hour
                ? 0
                : dateTimeObjectJson.minute;
            dateTimeObjectJson.second = !dateTimeObjectJson.second
                ? 0
                : dateTimeObjectJson.second;
            return this.each(function () {
                var $this = $(this),
                    setting = getSetting2($this);
    ...
    $.fn.MdPersianDateTimePicker = function (method) {
        if (methods[method]) {
            ...
            $.error(
                "Method " +
                    method +
                    " does not exist in jquery.Bootstrap-PersianDateTimePicker"
            );
            return false;
        }
    };
